...
async def debug_incoming(event):
    ...
    if event.sender_id != OWNER_ID:
        try:
            await client.send_message(OWNER_ID, f"📩 New message:\n{event.text[:200]}")
        except:
            pass
...
async def start(event):
    logger.info("Processing /start from %s", event.sender_id)
    user_id = event.sender_id
    ...
    if not is_verified(user_id):
        await event.reply(
            ...
            f"Please verify by joining our channel and group:\n"
...
async def verify(event):
    ...
    try:
        ...
        if ready:
            ...
            await event.reply("🎉 Congratulations! You also earned 100 credits for your inviter.")
    except Exception as e:
...
@client.on(events.NewMessage(pattern='/removeacc'))
...
@client.on(events.NewMessage(pattern='/aidm'))
...
@client.on(events.NewMessage(pattern='/schedule add'))
...
@client.on(events.NewMessage(pattern='/schedule del'))
...
@client.on(events.NewMessage(pattern='/schedule toggle'))
...
@client.on(events.NewMessage(pattern='/groups list'))
...
@client.on(events.NewMessage(pattern='/groups create'))
...
@client.on(events.NewMessage(pattern='/groups add'))
...
@client.on(events.NewMessage(pattern='/groups remove'))
...
@client.on(events.NewMessage(pattern='/groups delete'))
...
@client.on(events.NewMessage(pattern='/analytics'))
...
@client.on(events.NewMessage(pattern='/addedmembers'))
...
@client.on(events.NewMessage(pattern='/spintax'))
...
async def admin_cmd(event):
    ...
    if action == 'add':
        ...
        await event.reply(f"✅ User {target_id} is no longer admin.")
    elif action == 'ban':
        ban_user(target_id)
        ...
        await event.reply(f"✅ Set credits of {target_id} to {get_credits(target_id)} (added {amount})")
    else:
        await event.reply("❌ Unknown admin action.")
...
# ============================================================
# 🚀 MAIN – Railway ready with auto-reconnect
# ============================================================
