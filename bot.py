...
🚀 All features + fixed group handling + auto‑join + step‑by‑step
"""
...
# ============================================================
# 📂 DATABASE HELPER FUNCTIONS (ALL – included in full script)
# ============================================================
# (All helper functions – get_user, create_user, add_user_account, etc. – are included
#  in the complete file. I'm keeping them here for the full version.)
...
async def callback(event):
    ...
    if data == "menu_add":
        await event.edit("➕ **Add Members**\n\nType `/add` and follow the steps:\n1️⃣ Source group\n2️⃣ Target group\n3️⃣ How many to add\n\nTip: Just send the group username WITHOUT @ (e.g., PAK_EARN_HUB_1)", buttons=back_button())
        return
...
async def do_interactive_add(event, user_id, source, target, count):
    ...
    try:
        ...
        try:
            ...
            await event.reply(f"❌ Invalid groups. Please check:\nSource: @{source}\nTarget: @{target}\nError: {e2}")
    ...
    for i in range(count):
        try:
            ...
            try:
                ...
                # Try to get participants anyway
